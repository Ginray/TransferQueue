...
def test_default_transfer_timeout_is_finite():
    owner = _UcxOwnerThread(lambda: None, lambda: None)
    try:
        request = _FakeRequest(pending_polls=1, result="complete")
        future = owner.submit_request(
            lambda: request,
            lambda pending: pending.test(),
            None,
            DEFAULT_TRANSFER_TIMEOUT_SECONDS,
        )

        assert future.result(timeout=1) == "complete"
        assert DEFAULT_TRANSFER_TIMEOUT_SECONDS > 0
    finally:
        owner.stop()
...
def test_ucx_transfer_rejects_inconsistent_control_metadata():
    ...
    large = UcxTransfer(transfer_id, tag, 64 * 1024 * 1024 * 1024 + 1)
    large.validate()
...
def test_ucx_payload_transfer_fails_when_ucx_is_unavailable(monkeypatch):
    ...
    assert create_payload_transfer(local_ip="192.0.2.1") is None
    assert create_payload_transfer("zmq", local_ip="192.0.2.1") is None
