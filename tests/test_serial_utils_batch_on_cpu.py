...
def test_pack_into_accepts_non_byte_memoryview():
    item = memoryview(np.array([1, 2], dtype=np.int32))
    buf = bytearray(serial_utils.calc_packed_size([item]))
    serial_utils.pack_into(buf, [item])
    assert bytes(serial_utils.unpack_from(buf)[0]) == item.cast("B").tobytes()
