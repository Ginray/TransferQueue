...
"""Node-local UCX device selection hidden behind SimpleStorage configuration."""
...
def _discover_ucx_transports() -> frozenset[tuple[str, str]]:
    ...
    library_path = executable.parent.parent / "lib"
...
def _find_ucx_info() -> Path | None:
    ...
    # The wheel may load UCX from a private runtime directory that is not in
    # PATH.  Resolve the sibling bin directory from the loaded libucp mapping
    # without embedding a build-machine or vendor-specific absolute path.
    try:
