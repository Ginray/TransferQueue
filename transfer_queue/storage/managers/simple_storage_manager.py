...
class AsyncSimpleStorageManager(StorageManager):
    ...
    def __init__(self, controller_info: ZMQServerInfo, config: DictConfig):
        ...
        payload_transport = config.get("payload_transport", {}) or {}
        self.payload_transport_infos = config.get("payload_transport_infos", {}) or {}
        endpoints_ready = set(self.storage_unit_infos) == set(self.payload_transport_infos)
        self.data_plane = create_data_plane(
            bool(payload_transport.get("enabled", False)) and endpoints_ready,
    ...
    async def _put_to_single_storage_unit(
        ...
        # Preserve the legacy path exactly when optional payload transport is
        # disabled. Encoding here would otherwise duplicate the serialization
        ...
        if self.data_plane is not None:
            ...
            if len(payload) >= self.inline_threshold_bytes:
                await self._put_via_ucx(global_indexes, payload, len(frames), target_storage_unit, data_parser, socket)
                return
    ...
    async def _put_via_ucx(
        ...
        """PUT handshake: prepare receive -> Tagged send -> commit/store."""
        descriptor = self._new_descriptor(len(payload), frame_count)
        ...
        try:
            ...
            # The StorageUnit posts the tagged receive while processing PREPARE.
            # Start the UCX send immediately and receive READY concurrently so
            # the control-plane round trip does not sit on the critical path.
            ready_task = asyncio.ensure_future(socket.recv_multipart(copy=False))
            peer_address, peer_address_digest = self._peer_address(target_storage_unit)
            ucx_task = asyncio.wrap_future(
            ...
            ready = ZMQMessage.deserialize(await ready_task)
            ...
            pending_tasks = [
                task
                for task_name in ("ready_task", "ucx_task")
                if (task := locals().get(task_name)) is not None and not task.done()
            ]
            if pending_tasks:
            ...
            if prepared:
                await self._cancel_ucx_put(descriptor.transfer_id, target_storage_unit)
            raise

    async def _cancel_ucx_put(self, transfer_id: str, target_storage_unit: str) -> None:
        """Best-effort release of a remote UCX receive after a failed PUT."""
        await self._cancel_ucx_transfer(
    ...
    async def _get_from_single_storage_unit(
        ...
        if self.data_plane is not None:
            return await self._get_via_ucx(global_indexes, fields, target_storage_unit, socket)
        request_msg = ZMQMessage.create(
    ...
    async def _get_via_ucx(
        ...
        descriptor = self._new_descriptor(0, 0)
        remote_prepared = False
        receive_prepared = False
        receiver_address = self.data_plane.address
        prepare = ZMQMessage.create(
        ...
        try:
            await socket.send_multipart(prepare.serialize(), copy=False)
            # The StorageUnit may have accepted PREPARE even if READY is lost.
            # Use the initial transfer id for best-effort cleanup in that case.
            ...
            if ready.body.get("route") == "zmq_inline":
                remote_prepared = False
                return fields, ready.body["data"]
            ready_descriptor = PayloadDescriptor.from_dict(ready.body["descriptor"])
            if ready_descriptor.transfer_id != descriptor.transfer_id or ready_descriptor.tag != descriptor.tag:
            ...
            # commit and retired its pending protocol entry; UCX completion is
            # represented by the local receive Future. Wait for both so neither
            # control nor data-plane completion is left unobserved.
            receive_future = self.data_plane.finish_receive_future(descriptor)
            ...
            if len(frames) != descriptor.frame_count:
                raise RuntimeError(f"GET frame count mismatch: expected {descriptor.frame_count}, got {len(frames)}")
            result = decode(frames)
            return fields, result
        except BaseException:
            if receive_prepared:
                self.data_plane.cancel_receive(descriptor.transfer_id)
            if remote_prepared:
                await self._cancel_ucx_get(descriptor.transfer_id, target_storage_unit)
            raise

    async def _cancel_ucx_get(self, transfer_id: str, target_storage_unit: str) -> None:
        """Best-effort cancellation of a prepared, uncommitted GET."""
        await self._cancel_ucx_transfer(
    ...
    async def _cancel_ucx_transfer(
        self,
    ...
    def _new_descriptor(self, payload_bytes: int, frame_count: int) -> PayloadDescriptor:
    ...
    def _peer_address(self, storage_unit_id: str) -> tuple[bytes, str]:
        info = self.payload_transport_infos[storage_unit_id]
        if info.get("provider") != "ucx":
            raise RuntimeError(f"storage unit {storage_unit_id} does not advertise UCX")
        address = info.get("address")
        digest = info.get("address_digest")
        if not isinstance(address, bytes) or not isinstance(digest, str):
            raise RuntimeError(f"storage unit {storage_unit_id} has incomplete UCX address metadata")
    ...
    def close(self) -> None:
        """Close native transport before its Python/ZMQ ownership is released."""
        if self.data_plane is not None:
