...
class NixlPayloadTransfer(PayloadTransfer):
    ...
    def endpoint(self) -> TransferEndpoint:
        """Return the local NIXL agent metadata needed by remote senders."""
    ...
    def prepare_receive(self, descriptor: PayloadDescriptor) -> ReceiveToken:
        """Register receive storage and return its remote-write descriptors."""
    ...
    def receive(self, descriptor: PayloadDescriptor) -> Future[memoryview]:
        """Return the completed payload after detaching it from registered storage."""
        return self._runtime.receive(descriptor)

    def cancel_receive(self, transfer_id: str) -> None:
        """Cancel and release a prepared receive that will not be committed."""
        self._runtime.cancel_receive(transfer_id)

    def close(self) -> None:
        """Stop the NIXL runtime and release registered buffers."""
        self._runtime.close()
