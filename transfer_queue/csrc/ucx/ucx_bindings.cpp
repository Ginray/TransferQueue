...
static void receive_callback(void*, ucs_status_t, const ucp_tag_recv_info_t* info, void* user_data) {
  auto* state = static_cast<ReceiveState*>(user_data);
  if (info != nullptr) state->length = info->length;
}
...
class Request : public std::enable_shared_from_this<Request> {
 public:
  ...
  size_t size() const { return receive_state_ == nullptr ? 0 : receive_state_->length; }
 ...
 private:
  ...
  // native copy is needed for sends.
...
class Worker : public std::enable_shared_from_this<Worker> {
 public:
  ...
  std::shared_ptr<Request> post_receive(uint64_t tag, size_t length) {
    ...
    params.cb.recv = receive_callback;
    params.user_data = receive_state.get();
...
void Request::cancel() {
  if (complete_) return;
  if (request_ != nullptr) {
    void* request = request_;
    request_ = nullptr;
    try {
      worker_->cancel_request(request);
      worker_->wait_native(request, 1.0, "UCX request cancellation", true);
    } catch (...) {
      complete_ = true;
      throw;
    }
