...
def main() -> None:
    ...
    args = p.parse_args()
    if args.local_ip:
        dp = create_data_plane(True, local_ip=args.local_ip)
        if dp is None:
            raise RuntimeError("UCX auto discovery did not produce a data plane")
    else:
        dp = UcxDataPlane(args.timeout)
    descriptor = PayloadDescriptor(TRANSFER_ID, transfer_tag(TRANSFER_ID), args.size, 1)
    try:
        if args.role == "server":
            ...
            print("data_plane server PASS", flush=True)
        elif args.role == "server_exit":
            args.address_file.write_text(base64.b64encode(dp.address).decode())
            print("data_plane server_exit READY", flush=True)
            ...
            print("data_plane client PASS", flush=True)
        else:
            deadline = time.monotonic() + 30
            while not args.address_file.exists():
                if time.monotonic() > deadline:
            ...
            try:
                ...
                print("data_plane peer_exit PASS", flush=True)
            else:
