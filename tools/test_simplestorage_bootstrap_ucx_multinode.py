...
async def roundtrip_all_units() -> None:
    ...
    assert set(manager.payload_transfer_infos) == set(manager.storage_unit_infos)

    for offset, (storage_id, storage_info) in enumerate(manager.storage_unit_infos.items()):
        data_info = manager.payload_transfer_infos[storage_id]
        print(
            f"bootstrap unit metadata PASS id={storage_id} host={storage_info.ip} "
            f"address_len={len(data_info['endpoint']['data']['address'])}",
            flush=True,
        )
