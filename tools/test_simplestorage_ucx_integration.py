...
def make_manager(
    ...
    from transfer_queue.storage.data_plane import create_data_plane

    manager.data_plane = create_data_plane(enabled, local_ip=ray.util.get_node_ip_address())
    manager.payload_transport_infos = {server_info.id: data_info} if enabled else {}
    manager.inline_threshold_bytes = threshold
    if enabled and os.environ.get("TQ_INTEGRATION_WARMUP") == "1":
        from transfer_queue.storage.data_plane import address_digest

        manager.data_plane.warmup(data_info["address"], address_digest(data_info["address"]))
    return manager
...
async def run_case(enabled: bool) -> None:
    ...
    actor = SimpleStorageUnit.options(**actor_options).remote(
        ...
        payload_transport_enabled=enabled,
    )
    info = ray.get(actor.get_zmq_server_info.remote())
    data_info = ray.get(actor.get_payload_transport_info.remote())
    data_address_len = len(data_info["address"]) if data_info else None
    data_address_type = type(data_info["address"]).__name__ if data_info else None
    print(
    ...
    try:
        ...
        if manager.data_plane is not None:
