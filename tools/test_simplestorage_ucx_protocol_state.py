...
async def run_case(resource: str | None) -> None:
    ...
    try:
        ...
        assert ray.get(actor.get_payload_transfer_pending_counts.remote())["pending_gets"] == 1
        response = await request(
            info,
            control_message(
        ...
        assert ray.get(actor.get_payload_transfer_pending_counts.remote())["pending_gets"] == 1
        await manager._cancel_payload_get(transfer_id, info.id)
        assert ray.get(actor.get_payload_transfer_pending_counts.remote())["pending_gets"] == 0
        ...
        assert response.request_type == ZMQRequestType.PUT_GET_ERROR
        assert ray.get(actor.get_payload_transfer_pending_counts.remote())["pending_puts"] == 1
        response = await request(
            info,
            control_message(
                ZMQRequestType.PUT_DATA_CANCEL,
        ...
        assert ray.get(actor.get_payload_transfer_pending_counts.remote())["pending_puts"] == 0
