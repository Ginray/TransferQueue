...
async def run_roundtrip(label: str, index: int) -> None:
    ...
    assert manager.payload_transfer is not None
    assert manager.payload_transfer_infos
    storage_id, storage_info = next(iter(manager.storage_unit_infos.items()))
    endpoint = manager.payload_transfer_infos[storage_id]["endpoint"]
    assert endpoint["transport"] == "ucx"
    print(
        f"bootstrap metadata PASS storage={storage_id} host={storage_info.ip} "
        f"data_address_len={len(endpoint['data']['address'])}",
        flush=True,
    )
...
def main() -> None:
    ...
    try:
        ...
        assert final_conf.backend.SimpleStorage.payload_transfer_infos
