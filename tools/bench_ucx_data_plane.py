#!/usr/bin/env python3
"""Measure the TQ UCX Host data plane without Ray or SimpleStorage."""
...
from transfer_queue.storage.data_plane import PayloadDescriptor, UcxDataPlane, address_digest, transfer_tag
...
def descriptor(size: int, iteration: int) -> PayloadDescriptor:
    transfer_id = f"tq-bench-{size}-{iteration}"
    return PayloadDescriptor(transfer_id, transfer_tag(transfer_id), size, 1)
